#!/usr/bin/env python3
"""Lien Ch.10 / 11 / 13 / 14 / 16 entry signal from OANDA candles (no MCP).

Chapter 10 double zeros (D + M15), 11 Waiting for the Deal (D + M15), 13 Fader
(D + H1), 14 20-day breakout (D), 16 perfect order (D). Prints a signal JSON.
Research only; no orders. See docs/LIEN_FX_STRATEGIES.md.
...
async def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.chapter in (10, 11):
            ltf = (
                double_zeros.resolve_ltf(args.ltf_granularity)
                if args.chapter == 10
                else waiting_deal.resolve_ltf(args.ltf_granularity)
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Lien Ch.10/11/13/14/16 entry signal (double zeros, waiting for "
            "the deal, fader, 20-day breakout, perfect order)."
        )
    )
    parser.add_argument(
        "--chapter",
        type=int,
        required=True,
        help="10 (double zeros), 11 (waiting for the deal), 13 (fader), 14 (20-day), or 16 (perfect order).",
    )
    ...
    parser.add_argument(
        "--ltf-granularity",
        default="H1",
        help="Lower TF (Ch.10/11 remap H1→M15; Ch.13 default H1).",
    )

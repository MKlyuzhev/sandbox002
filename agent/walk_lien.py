...
Causal paper walk for encoded Lien chapters 9 / 10 / 11 / 13 / 14 / 16. Single-TF
event engines (9, 14, 16) step the exported granularity; Ch.13 Fader steps H1
against a daily ADX gate; Ch.10/11 step M15 against a daily Ch.7 gate.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Causal Lien paper walk for chapters 9, 10, 11, 13, 14, or 16. "
            "Research only; no broker orders."
        )
    )
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="9 (dbb), 10 (double_zeros), 11 (waiting_deal), 13 (fader), 14 (breakout20), or 16 (perfect_order).",
    )
    ...
    parser.add_argument(
        "--ltf-granularity",
        default="H1",
        help="Lower TF for Ch.10/11 (remap H1→M15) and Ch.13 (default H1).",
    )
    parser.add_argument(
        "--lookback",
        type=int,

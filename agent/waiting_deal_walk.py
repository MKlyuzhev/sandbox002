"""Causal Ch.11 Waiting-for-the-Deal walk: step M15, daily Ch.7 gate, first-fire.

Dual series like ``fader_walk`` (causal HTF alignment via ``htf_index_as_of``).
Geometry is calendar + LTF OHLC (no LTF regime). First-fire on the reverse bar.
...
from agent.engines import waiting_deal as deal_mod
...
def _eval_deal_at(
    ...
    *,
    ...
    """Return (waiting_deal_out, htf_analysis) for LTF bar ``i``."""
...
def waiting_deal_decisions(
    ...
    *,
    ...
    hunt_pips: int = deal_mod.HUNT_PIPS,
    clock: str = "frankfurt",
) -> list[dict[str, Any]]:
    ...
    for i in range(start_index, len(ltf_series)):
        out, htf_analysis = _eval_deal_at(
            htf_series,
            ltf_series,
            i,
            lookback=lookback,
...
def walk_waiting_deal(
    ...
    *,
    ...
    """Walk LTF bars with one open Waiting-for-the-Deal position at a time."""
    if lookback < indicators.MIN_BARS:
        raise regime_walk.WalkError(
            f"lookback must be >= {indicators.MIN_BARS}; got {lookback}"
        )

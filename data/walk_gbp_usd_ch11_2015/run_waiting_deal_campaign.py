"""Causal Ch.11 Waiting-for-the-Deal campaign: GBP_USD M15, 2015-now.

Research only; no orders. Fetches OANDA D + M15 once (cached), walks with a
cached daily Ch.7 gate. Fill at decision-bar close; 2% equity; vppu=1.0.
News/FOMC is not filtered. Clock is DST-aware Frankfurt→London.
"""
...
D_CACHE = OUT / "gbp_usd_d.json"
M15_CACHE = OUT / "gbp_usd_m15.json"
TRADES_PATH = OUT / "waiting_deal_trades.json"
SUMMARY_PATH = OUT / "waiting_deal_campaign.json"
...
def build_htf_cache(htf_bars: list[dict]) -> dict[str, dict]:
    """Causal Ch.7 snapshot keyed by daily bar time."""
    cache: dict[str, dict] = {}
...
def htf_as_of(cache: dict[str, dict], times: list[str], t: str) -> dict | None:
    """Last daily analysis with time <= t."""
    import bisect
...
async def main() -> int:
    ...
    pip = 0.0001
    print("microstructure day outcomes…", flush=True)
    ...
    summary = {
        ...
        "fill_mode": "close",
        "note": (
            "Causal Ch.11 paper walk. Daily Ch.7 gate (fade_range or "
            "breakout_watch); M15 Frankfurt–London power hour, ≥25-pip hunt, "
            "reverse through opposite rail. 2R tickets, 2% equity. "
    ...
    print(json.dumps({k: summary[k] for k in (
        "instrument", "trade_count", "equity", "exits", "by_side", "clock_stats"

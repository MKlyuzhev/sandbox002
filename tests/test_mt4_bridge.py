...
class TestRegimeObjects(unittest.TestCase):
    def test_regime_to_objects_has_bands_mas_and_label(self) -> None:
        ...
        self.assertIn("sbox.regime.channel.upper", names)
        self.assertIn("sbox.regime.channel.lower", names)
        ch_up = next(o for o in objects if o["name"] == "sbox.regime.channel.upper")
        ch_lo = next(o for o in objects if o["name"] == "sbox.regime.channel.lower")
        self.assertEqual(ch_up["type"], "trend")
        self.assertFalse(ch_up.get("ray"))
        self.assertEqual(ch_up["width"], 2)
        self.assertGreater(ch_up["p1"], ch_lo["p1"])
        ...
        self.assertTrue(any("channel rails" in o["text"] for o in legend))

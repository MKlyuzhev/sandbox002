...
| `double_zeros` | 10 | `fade_range` | `agent.run`, `agent.walk_lien --chapter 10`, `entry_lien` |
...
Ch. 12 and 15 remain documentation-only in [LIEN_FX_STRATEGIES.md](LIEN_FX_STRATEGIES.md).
...
| `entry_dbb` / `entry_lien` as MCP | **Done** — Ch. 9 `entry_dbb`; Ch. 10/11/13/14/16 `entry_lien` | Ch. 12/15 still unencoded |
...
4. Encode remaining chapters (12, 15) when `breakout_watch` policy is honest.
...
| `entry_lien` | Ch. 10/11/13/14/16 (double zeros, waiting for the deal, fader, 20-day, perfect order) | **Done** — MCP + `scripts/entry_lien.py` |
...
- **Add:** OANDA candles + Ch. 7 regime + entry engines (Ch. 8/9/10/11/13/14/16 + Ch. 7 fallback)
...
(Ch. 9/10/11/13/14/16). MT4 Strategy Tester bridge: `agent.tester_backtest`. Ch. 12
and 15 remain documentation-only.
...
| Entry engines Ch. 8/9/10/11/13/14/16 + Ch. 7 fallback | `agent/engines/` |
...
2. Remaining Lien chapters 12, 15 (`breakout_watch` policy)
